...
The default plugin spec leaves jupynium's built-in keymaps intact (no `keys = {...}` override). The wizard offers five optional keymaps — `<leader>jj` "run cell, move to next", `<leader>jJ` "run from this cell to end", `<leader>j<CR>` "run cell", `<leader>jn` "next cell", `<leader>jp` "previous cell" — each prompted individually during install. Controlled by `WIZARD_NVIM_KEYMAP` env (or `--nvim-keymaps all|none|jj,jJ,jcr,jn,jp`). The default for `--yes` is `none` (conservative). After installation, `--update-keymaps` regenerates the plugin file with new keymap choices without re-running the full wizard (it detects the existing load mode from the plugin file). Other extras (`<C-CR>`/`<S-CR>`, buffer-local `<C-x>`/`X`) live as copy-paste snippets in the README — don't bake them into the generated template.
