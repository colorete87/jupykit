...
The default plugin spec leaves jupynium's built-in keymaps intact (no `keys = {...}` override). The wizard offers three optional keymaps — `<leader>jj` "execute cell and advance", `<leader>jk` "go to previous cell", `<leader>j<CR>` "execute cell" — controlled by `WIZARD_NVIM_KEYMAP=yes|no` env (or `--nvim-keymaps yes|no`). The default for `--yes` is `no` (conservative). Other extras (`<C-CR>`/`<S-CR>`, buffer-local `<C-x>`/`X`) live as copy-paste snippets in the README — don't bake them into the generated template.
