...
The default plugin spec leaves jupynium's built-in keymaps intact (no `keys = {...}` override). The wizard offers three optional keymaps — `<leader>jj` "execute cell and advance", `<leader>jk` "go to previous cell", `<leader>j<CR>` "execute cell" — each prompted individually during install. Controlled by `WIZARD_NVIM_KEYMAP` env (or `--nvim-keymaps all|none|jj,jk,jcr`). The default for `--yes` is `none` (conservative). After installation, `--update-keymaps` regenerates the plugin file with new keymap choices without re-running the full wizard (it detects the existing load mode from the plugin file). Other extras (`<C-CR>`/`<S-CR>`, buffer-local `<C-x>`/`X`) live as copy-paste snippets in the README — don't bake them into the generated template.
