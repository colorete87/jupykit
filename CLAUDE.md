...
- **Modes** (mutually exclusive, default `run`): `run`, `install`, `install-nvim`, `update-keymaps`, `uninstall`, `doctor` → dispatched to `cmd_run`, `cmd_install`, `cmd_install_nvim`, `cmd_update_keymaps`, `cmd_uninstall`, `cmd_doctor`.
...
The generated plugin spec disables jupynium's default keybindings (`use_default_keybindings = false` and `textobjects.use_default_keybindings = false` in `opts`) so the `<leader>j` prefix is fully ours — otherwise jupynium's `<space>jj` (Go to current cell, set via a buffer-local `BufWinEnter` autocmd) overwrites our `<leader>jj`. The `keys = {...}` block then unconditionally re-adds the non-conflicting builtins worth keeping (`<PageUp>`/`<PageDown>` for notebook scroll, `[j`/`]j` for cell navigation, `aj`/`ij`/`aJ`/`iJ` text objects), plus the eight optional `<leader>j*` actions selected by the user: `jj` (run+advance), `jJ` (run from here to end), `jX` (run all cells), `j<CR>` (run cell), `jx` (execute selected), `jc` (clear outputs), `jk` (kernel hover), `jo` (toggle output scroll). Each optional keymap is prompted individually. Controlled by `WIZARD_NVIM_KEYMAP` env (or `--nvim-keymaps all|none|jj,jJ,jX,jcr,jx,jc,jk,jo`). The default for `--yes` is `none` (conservative). After installation, `--update-keymaps` regenerates the plugin file with new keymap choices without re-running the full wizard (it detects the existing load mode from the plugin file).
