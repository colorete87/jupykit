...
_generate_plugin_lua() {
  ...
  # Always-on keymaps: re-implement the jupynium builtins we want to keep.
  # We disable use_default_keybindings (in opts below) so the <leader>j prefix
  # is fully ours; PageUp/PageDown and the [j/]j/aj/ij/aJ/iJ textobjects are
  # added back here unconditionally because they don't collide with anything.
  local keys_entries='
      { "<PageUp>",   "<cmd>JupyniumScrollUp<cr>",   mode = "", desc = "Jupynium scroll up" },
      { "<PageDown>", "<cmd>JupyniumScrollDown<cr>", mode = "", desc = "Jupynium scroll down" },
      { "[j", "<cmd>lua require'\''jupynium.textobj'\''.goto_previous_cell_separator()<cr>", mode = { "n", "x", "o" }, desc = "Previous Jupynium cell" },
      { "]j", "<cmd>lua require'\''jupynium.textobj'\''.goto_next_cell_separator()<cr>",     mode = { "n", "x", "o" }, desc = "Next Jupynium cell" },
      { "aj", "<cmd>lua require'\''jupynium.textobj'\''.select_cell(true,  false)<cr>", mode = { "x", "o" }, desc = "Around Jupynium cell" },
      { "ij", "<cmd>lua require'\''jupynium.textobj'\''.select_cell(false, false)<cr>", mode = { "x", "o" }, desc = "Inside Jupynium cell" },
      { "aJ", "<cmd>lua require'\''jupynium.textobj'\''.select_cell(true,  true)<cr>",  mode = { "x", "o" }, desc = "Around Jupynium cell (with next separator)" },
      { "iJ", "<cmd>lua require'\''jupynium.textobj'\''.select_cell(false, true)<cr>",  mode = { "x", "o" }, desc = "Inside Jupynium cell (with next separator)" },'
  ...
  if [[ ",$keymaps," == *",jx,"* ]]; then
    keys_entries+='
      { "<leader>jx", "<cmd>JupyniumExecuteSelectedCells<cr>",            mode = { "n", "x" }, desc = "Execute selected cells" },'
  fi
  if [[ ",$keymaps," == *",jc,"* ]]; then
    keys_entries+='
      { "<leader>jc", "<cmd>JupyniumClearSelectedCellsOutputs<cr>",       mode = { "n", "x" }, desc = "Clear selected cells outputs" },'
  fi
  if [[ ",$keymaps," == *",jk,"* ]]; then
    keys_entries+='
      { "<leader>jk", "<cmd>JupyniumKernelHover<cr>",                     mode = "n",          desc = "Inspect variable under cursor" },'
  fi
  if [[ ",$keymaps," == *",jo,"* ]]; then
    keys_entries+='
      { "<leader>jo", "<cmd>JupyniumToggleSelectedCellsOutputsScroll<cr>", mode = { "n", "x" }, desc = "Toggle output scroll" },'
  fi

  local keys_block="    keys = {${keys_entries}
    },"

  cat <<EOF
-- Generated by jupykit $JUPYKIT_VERSION on $(date -Iseconds)
-- Edit freely; jupykit backs up before overwriting on the next --install.
-- Jupynium's default <space>-prefixed keymaps are disabled; the kept
-- builtins (PageUp/PageDown, [j/]j, aj/ij/aJ/iJ) are re-added in the
-- keys block below alongside the optional <leader>j actions you selected.
return {
  {
    ...
    opts = function()
      ...
      return {
        ...
        -- Own the <leader>j namespace; we re-add the kept builtins in keys below.
...
# Prompt for individual keymaps. Echoes comma-separated list (e.g. "jj,jJ,jcr,jx,jc,jk,jo")
# or empty string. Honors WIZARD_NVIM_KEYMAP env var:
#   "all"/"yes" → all keymaps, "none"/"no" → none, or comma-separated subset.
#
# Jupynium's built-in keybindings are disabled in the generated spec (the
# <leader>j prefix is fully ours). PageUp/PageDown, [j/]j, and aj/ij/aJ/iJ
# are re-added unconditionally; the entries below are the user-facing
# actions worth prompting about.
_prompt_keymaps() {
  ...
  echo "  Optional <leader>j keymaps (PageUp/PageDown, [j/]j, aj/ij are always added):" >&2
  local result=""
  local letter
